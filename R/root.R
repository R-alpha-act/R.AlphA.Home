...
#' is located, or optionally the full file path including filename.
#'
...
#'
#' @param includeFName Logical. If \code{FALSE} (default), returns only the directory path.
#' If \code{TRUE}, returns the full file path including the filename.
#'
#' @return A character string representing either:
#' \itemize{
#'   \item The absolute path of the directory containing the current source file (if \code{includeFName = FALSE})
...
root <- function(includeFName = FALSE){
	...
	return(if(includeFName) sourceLoc else rootDirname)
}
